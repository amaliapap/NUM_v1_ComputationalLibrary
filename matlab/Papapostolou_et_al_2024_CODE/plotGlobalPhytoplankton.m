...
%         % post5=t5.Position(2);
 ...
 t6=nexttile();
cbar=panelGlobal(sim.x, sim.y, mean(sim.DiatomratioPhyto(ixTime,:,:),1), sTitle="a. Diatom:phytoplankton ratio", sUnits="", sProjection=options.sProjection);
        cbar.Visible='off';
        clim([0 1])
        colormap(t6,ccmap2)
        cbar=colorbar('horizontal');
...
t7=nexttile();
cbar=panelGlobal(sim.x, sim.y, 1-mean(sim.Passiveratio(ixTime,:,:),1), sTitle="b. Active:total copepod ratio", sUnits="", sProjection=options.sProjection);
        cbar.Visible='off';
        clim([0 1])
        colormap(t7,ccmap2);
        set(gca,'YTickLabel',[]);
