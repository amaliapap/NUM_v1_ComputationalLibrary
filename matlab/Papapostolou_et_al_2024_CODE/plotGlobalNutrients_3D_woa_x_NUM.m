%% 2 FIGURES
% nutrient global concentrations downloaded from https://www.ncei.noaa.gov/access/world-ocean-atlas-2023/
function plotGlobalNutrients_3D_woa_x_NUM(sim)
%%
%addpath('woa/')

ixTime = sim.t > (sim.t(end)-365);
NO3 = squeeze(mean(sim.N(ixTime,:,:,1),1)); % annual average of the last year
NO3( NO3<0 ) = 0;
Si = squeeze(mean(sim.Si(ixTime,:,:,1),1));   % annual average of the last year
DOC = squeeze(mean(sim.DOC(ixTime,:,:,1),1));   % annual average of the last year

if exist('../../data/no3_woa_3D_interp.mat')
    showData = true;
    % Annual WOA data
    load('../../data/no3_woa_3D_interp.mat')
